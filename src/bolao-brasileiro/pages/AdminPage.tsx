...
import { formatMatchDateTime } from '../../bolao/lib/dates';
...
export function AdminPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleteId(id);
    await supabase.from('brasileirao_matches').delete().eq('id', id);
    setDeleteId(null);
    refetchMatches();
  };
  ...
  return (
    <div className="bolao-page bolao-page--wide">
      <h1 className="bolao-page__title">⚙️ Painel Admin — Brasileirão</h1>
                ...
                <tbody>
                  {matches.map(m => (
                    <tr key={m.id}>
                      ...
                      <td className={styles.tdMuted}>{roundLabel(m.round_number)}</td>
